...
export const ProjectDetailPage: React.FC = () => {
  ...
  return (
    <div className="py-10 sm:py-16 md:py-20 max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 space-y-12 sm:space-y-16">
      ...
      {/* Full Visual Mockup Showcase Banner */}
      <div className="w-full rounded-2xl sm:rounded-3xl overflow-hidden shadow-xl border border-[#192841]/15">
        <ProjectMockupPreview project={project} variant="detail" />
      </div>
      ...
      {/* Previous & Next Footer Navigation (Clean inline style) */}
      <div className="pt-6 border-t border-[#192841]/10 flex flex-col sm:flex-row items-center justify-between gap-4">
        <Link
          to={`/projects/${prevProject.id}`}
          className="inline-flex items-center gap-2 text-sm font-bold text-[#192841] hover:text-[#233758] link-hover group w-full sm:w-auto justify-start"
        >
          <ArrowLeft className="w-4 h-4 transition-transform duration-200 group-hover:-translate-x-1" />
          <span>{prevProject.number} — {prevProject.title}</span>
        </Link>

        <Link
          to={`/projects/${nextProject.id}`}
          className="inline-flex items-center gap-2 text-sm font-bold text-[#192841] hover:text-[#233758] link-hover group w-full sm:w-auto justify-end"
        >
